...
void Game::play()
{
	...
	do {
		...
		initState();
		displayPrompt("\t\tPress the Enter key to begin Space Invaders Light\n");
		displayPrompt("Note: up arrow or space is shoot");
		...
		while (m_hero->getHP() > 0) {
			...
			if (getCharIfAny(keyPressed)) {
				...
				if (keyPressed == ARROW_UP || keyPressed == ' ') {
